...
let pay_notif = make_notif "payer" "payee" "300 USD" false
let friend_notif = make_notif_friend "user1" false
let pay_notif_accepted = make_notif "payer" "payee" "300 USD" true
let friend_notif_accepted = make_notif_friend "user1" true

let notif_inbox =
  [ friend_notif; pay_notif; pay_notif_accepted; friend_notif_accepted ]
let acc_notif = acc_new_inbox acc1 notif_inbox
let acc_notif = acc_new_inbox acc3 [pay_notif; friend_notif]
let acc3_notif_clear = notif_clear acc3
let acc3_add_notif = add_notification acc4 friend_notif
let notification_tests =
  [
    ( "pay_notif is a payment request" >:: fun _ ->
      assert_equal "true"
        (string_of_bool (notif_type pay_notif))
        ~printer:(fun x -> x) );
    ( "friend_notif is a friend request" >:: fun _ ->
      assert_equal "false"
        (string_of_bool (notif_type friend_notif))
        ~printer:(fun x -> x) );
    ( "pay_notif's payer is \"payer\"" >:: fun _ ->
      assert_equal "payer" (notif_payer pay_notif) ~printer:(fun x -> x) );
    ( "friend_notif's payer is \"user1\"" >:: fun _ ->
      assert_equal "user1" (notif_payer friend_notif) ~printer:(fun x -> x) );
    ( "pay_notif's payee is \"payee\"" >:: fun _ ->
      assert_equal "payee" (notif_payee pay_notif) ~printer:(fun x -> x) );
    ( "pay_notif's amount is \"300 USD\" " >:: fun _ ->
      assert_equal "300 USD" (notif_amount pay_notif) ~printer:(fun x -> x) );
    ( "pay_notif's acceptance status " >:: fun _ ->
      assert_equal "false"
        (string_of_bool (notif_accepted pay_notif))
        ~printer:(fun x -> x) );
    ( "friend_notif's acceptance status" >:: fun _ ->
      assert_equal "false"
        (string_of_bool (notif_accepted friend_notif))
        ~printer:(fun x -> x) );
    ( "pay_notif's acceptance status " >:: fun _ ->
      assert_equal "true"
        (string_of_bool (notif_accepted pay_notif_accepted))
        ~printer:(fun x -> x) );
    ( "friend_notif's acceptance status" >:: fun _ ->
      assert_equal "true"
        (string_of_bool (notif_accepted friend_notif_accepted))
        ~printer:(fun x -> x) );
    ( "pay_notif's string form" >:: fun _ ->
      assert_equal "payee requested payer 300 USD and transaction not accepted"
        (string_of_notif pay_notif) ~printer:(fun x -> x) );
    ( "friend_notif's string form" >:: fun _ ->
      assert_equal "user1 requested following- status: not accepted"
        (string_of_notif friend_notif) ~printer:(fun x -> x) );
    ( "pay_notif's string form" >:: fun _ ->
      assert_equal "payee requested payer 300 USD and transaction accepted"
        (string_of_notif pay_notif_accepted) ~printer:(fun x -> x) );
    ( "friend_notif's string form" >:: fun _ ->
      assert_equal "user1 requested following- status: accepted"
        (string_of_notif friend_notif_accepted) ~printer:(fun x -> x) );
    ( "acc1's notif inbox's length" >:: fun _ ->
      assert_equal "4" (string_of_int (length_notif acc1)) ~printer:(fun x -> x));
  
    (  ("acc2's notif inbox's length")  >:: fun _ ->
        assert_equal "0" (string_of_int (length_notif acc2))
     ~printer:(fun x -> x) );


     (  ("acc3's notif inbox's length after clearing")  >:: fun _ ->
      assert_equal "0" (string_of_int (length_notif acc3))
   ~printer:(fun x -> x) );

   (  ("acc3's notif inbox's length after adding a notification")  >:: fun _ ->
    assert_equal "1" (string_of_int (length_notif acc4))
 ~printer:(fun x -> x) );
...
let transaction_tests =
  ...
  let one_friend = add_friend acc1 "user1" 
  let two_friend = add_friend acc2 "user2"
  let two_friend_two = add_friend acc2 "user3"
  let friend_test = [ 
    ("original friend list is empty" >:: fun _ ->
    assert_equal "\nFriends List\n\n"
      (display_friends transaction_test_acc) ~printer:(fun x -> x));

      ("acc1's friend list length" >:: fun _ ->
        assert_equal "1"
          (string_of_int (List.length (friend_list acc1))) ~printer:(fun x -> x));


          ("acc2's friend list length" >:: fun _ ->
            assert_equal "2"
            (string_of_int (List.length (friend_list acc2))) ~printer:(fun x -> x));
